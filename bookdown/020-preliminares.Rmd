...
Considermos el intervalo $[0,1]$, lo dividimos en tres subintervalos de longitud $\frac{1}{3}$, a saber, $J_{1,1}=[0,\frac{1}{3}]$, $I_{1,1}=(\frac{1}{3},\frac{2}{3})$ y $J_{1,2}=[\frac{2}{3},1]$. LLamamos $C_{1}=J_{1,1}\cup J_{1,2}$, que es unión de conjuntos disjuntos. Además $C_{1}=[0,1]\backslash I_{1,1}$.\\

Luego, los dos intervalos cerrados $J_{1,1}$ y $J_{1,2}$ se dividen nuevamente en tres subintervalos de longitud $\frac{1}{3^{2}}$; estos son, $J_{21}=[0,\frac{1}{3^{2}}]$, $I_{2,1}=(\frac{1}{3^{2}},\frac{2}{3^{2}})$, $J_{2,2}=[\frac{2}{3^{2}},\frac{3}{3^{2}}]$, al dividir $J_{1,1}$ y $J_{2,3}=[\frac{6}{3^{2}},\frac{7}{3^{2}}]$, $I_{2,2}=(\frac{7}{3^{2}},\frac{8}{3^{2}})$ y $J_{2,4}=[\frac{8}{3^{2}},1]$, que resultan de dividir $J_{1,2}$. Al igual que en el primer paso, descartamos los dos intervalos abiertos $I_{2,1}$ e $I_{2,2}$. LLamamos $C_{2}=J_{2,1}\cup J_{2,2}\cup J_{2,3}\cup J_{2,4}$. Además $C_{2}=[0,1]\backslash (E_{1}\cup E_{2})$ donde $E_{i}=\bigcup\limits_{k=1}^{2^{n-1}} I_{n,k}$.\\
...
En el $n$-\'esimo paso se tienen $2^{n}$ intervalos cerrados $J_{n,i}$, $i=1,\cdots,2^{n}$ y se han descartado $2^{n-1}$ intervalos abiertos $I_{n,i}$ $i=1,\cdots,2^{n-1}$. Hemos construído el conjunto $C_{n}=\bigcup\limits_{k=1}^{2^{n}} J_{n,k}$, de forma que $C_{1}\supseteq C_{2}\supseteq \cdots \supseteq C_{n}$. Adem\'as, $C_{n}=[0,1]\backslash\bigcup\limits_{k=1}^{n} E_{k}$ donde $E_{k}=\bigcup\limits_{k=1}^{2^{n-1}}$.\\
...
```{remark}
Se puede demostrar que el conjunto de Cantor se puede poner en correspondencia $1$ a $1$ con el intervalo $[0,1]$. Por lo tanto, $C$ es un conjunto no numerable. Para esto es útil notar que si $x\in C$, entonces $x$ es un número real tal que $0\leq x\leq 1$ y $x=\sum\limits_{n=0}^{\infty} \frac{a_{n}}{3^{n}}$ donde $a_{n}=0$ o $a_{n}=2$ para todo $n\in\mathbb{N}$.
